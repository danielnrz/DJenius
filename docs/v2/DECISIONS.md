...
## D057 - Autonomous template choice is a constrained explainable abstaining layer
**Decision:** `reference_selector.py` may choose only F, C3, B8, D2, or `NO_SUITABLE_TEMPLATE`. It searches bounded nearby source and target downbeat windows per archetype, retains every failed cue condition, and requires both template eligibility and a named musical-story rule. Cross-template choice is an explicit decision tree; cue choice is an exposed lexicographic ordering of named evidence. Neither uses one aggregate compatibility score, Audition Lab, Candidate Composer, Set Director, or an LLM.
**Why:** human generalization proved that the same choreography succeeds or fails based on pair context and exact launch point. The system must explain why a behavior and cues fit, and abstain when none of the four proven stories applies, before broader planning is allowed to rely on it.
**Rejected:** choosing the largest legacy `fit_score`, rendering all eligible templates and waveform-ranking them, silently moving to a distant unrelated target section, inventing a fifth fallback family, or forcing a transition to meet an output count.

## D058 - Technical eligibility is not permission to perform
**Decision:** reference selection now has two explicit gates. A template/cue may be technically `eligible`, but it is selectable only when every named archetype-specific `USABLE_FOR_PERFORMANCE` check passes. A pair separately declares `PAIR_TRANSITIONABLE = YES` only when at least one frozen template clears that stronger floor; otherwise selection must abstain and preserve pair-level reasons. Future set autonomy must jointly choose `(current track, candidate next track, approved template, valid cues)` rather than commit to a next track and then force a transition.
**Why:** blind listening rejected all four first-round selections even though each satisfied the former story gate. Calibration also showed cue-search defects: the old selector missed two accepted source cues and entirely missed the accepted F intro pickup. After selector-only cue corrections it rediscovers all four accepted generalization choices exactly, while the four failed blind pairs have no credible alternative and all fail the stronger performance floor. False-positive performance selection is therefore more harmful than conservative abstention.
**Rejected:** treating any eligible template as usable, collapsing the stronger floor into one score, rendering unqualified counterfactuals, changing frozen choreography to rescue a pair, or reconnecting Set Director before joint pair/transition selection is designed and human-gated.

## D059 - Set-level autonomy first reconnects as an isolated joint hard-gated planner
**Decision:** the first set-level pilot lives in `joint_reference_set.py`, not the legacy Set Director. It searches bounded paths whose edges jointly bind current track, candidate next track, one frozen F/C3/B8/D2 instance, and its source/target cues. Set-flow gates and `PAIR_TRANSITIONABLE`/`USABLE_FOR_PERFORMANCE` are all mandatory. The deterministic opening rule selects the first musically suitable opener for which a complete hard-gated path exists. Transition diversity is considered only after performance and set-flow evidence and may never make an unqualified edge usable. The companion renderer executes each frozen transition as one intact window, then continues the target's source clock through sample-aligned natural playback; interstitial joins occur after declared postlanding/tail lifetimes.
**Why:** the failed blind selector round demonstrated that choosing a song first and forcing one of the four templates afterward produces false positives. The pilot must test the joint decision directly without reopening the older generic candidate/audition policy. A continuous set also requires each middle track's incoming consumption, establishment, and outgoing launch to be chronologically coherent rather than three independent transition WAVs concatenated after rendering.
**Rejected:** restoring the old unrestricted Set Director, using Audition Lab to overrule hard reference eligibility, assigning one weighted compatibility score, lowering the floor to complete four tracks, enforcing archetype variety as a hard rule, or splicing three unrelated finished transition files.

## D060 - Set-flow suitability is independent stateful evidence, not transitionability
**Decision:** a joint candidate must independently pass `PERFORMANCE_FEASIBILITY` and `SET_FLOW_SUITABILITY`. The latter derives an interpretable musical-role profile from existing acoustic, groove, section, optional semantic, and optional lyric evidence; carries recent energy, dance function, rhythmic character, reliable mood, declared set phase, and intentional-contrast count; and classifies each step as coherent continuation, supported intentional shift, or unjustified discontinuity. Semantic mood/style is actionable only above an explicit `.55` reliability floor. A reset may communicate one phase-supported contrast, but changing a state label cannot rehabilitate an adjacency already rejected by direct human listening.
**Why:** the first joint pilot proved that technically executable transitions can connect tracks with incompatible emotional or functional purposes. Its old evidence had no mood, style, rhythmic role, or trajectory state. The human-described affect contrast could not be reproduced reliably by the current local semantic model, but acoustic rhythmic evidence still exposed the mismatch; recording that limitation is more truthful than inventing confidence. Short-term state makes a large change valid only when the declared arc and choreography actually explain it.
**Rejected:** same-mood-only matching, a single opaque flow score, treating BPM/key compatibility as set story, allowing an archetype to override failed set-flow gates, relabeling a known bad adjacency as an intentional phase change, or adding a new mood-model dependency for this pilot.

## D061 - Musical bridgeability uses cue-local relative context, not semantic labels
**Decision:** classify each adjacency as `NATURAL_CONTINUATION`, `INTENTIONAL_BRIDGEABLE_CONTRAST`, or `UNJUSTIFIED_DISCONTINUITY`. Reuse the existing local CLAP whole-track embedding and cached representative-window score distributions as pool-relative musical-territory evidence. Calibrate distance bands from all unlabelled pool pairs/windows, never human labels. A large contrast is bridgeable only when the declared phase and contrast budget support it, frozen F is performance-usable, at least two explicit continuity anchors remain, cue evidence is available, and cue-local style/intensity distributions do not show an unresolved multi-axis split. C3/B8/D2 remain natural-continuation tools under their frozen contracts.
**Why:** Pilot 2 transitions 2 and 3 were locally executable but contextually wrong. Whole-track embedding and reliable mood/style labels alone did not separate them. The already-cached cue-near distributions did, without claiming that a score means “sad,” “celebratory,” or any particular genre.
**Rejected:** adding a new classifier before exhausting existing evidence; treating energy or genre alone as context; allowing any F reset to bridge any contrast; fitting thresholds to the small human-labelled sample.

## D062 - Cue search applies the performance floor before cue ranking
